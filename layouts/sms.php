<?php
    ...
    if((!isset($_SESSION['USUARIO'])) && ($_SESSION['USUARIO']['VALIDA'] != true)){
		...
		include_once("../classes/classSmsSender.php");
		...
		/* Consultas para informações de SMS */
	
	
		$sSql_SMS = "SELECT id, paciente,  horario_consulta,  p.nome, cabecalho, mensagem, status, resposta
					FROM `sms`, pessoa as p
					WHERE sms.paciente = p.id_pessoa
					and instancia = 0;";
					
		$sSQL_StatusSMS = "SELECT id_status, descricao FROM `sms_status`;";
	
		$pers = new Persistencia();
		$pers->bExecute($sSQL_StatusSMS);
		$cont = 0;
		$lista_sms_status = "";
		...
		while($cont < $pers->getDbNumRows()){   
			...
			$desc = ucfirst($desc)." {".$vet_result['id_status']."}";
			$lista_sms_status .= "<span class=\"displayInline\">";
			$lista_sms_status .= "<input onclick='".$vet_result['id_status']."' type='checkbox' id='_".$vet_result['id_status']."' name='".$vet_result['id_status']."' value='".$vet_result['id_status']."'/>".
			"<label onClick='document.getElementById(\"_".$vet_result['id_status']."\").click()' for='".$vet_result['id_status']."'>".$desc."</label>";
			$lista_sms_status .= "</span>";
			$cont++;
		}
...
<?php include_once("include/menu.php") ?>
    <div id="conteudo">
        <div id="dropshadow">
            <div id="breadcrumb">
                <ul>
                    <li><span class="breadcrumbEsquerda bcrumbSelect"></span><h2 class="bcrumbAtivo bcrumbSelect">sms</h2><span class="breadcrumbDireita bcrumbSelect"></span></li>
                </ul>
            ...
            <div id="container" class="clearfix">
                ...
                <div id="boxfiltroTratamentos" class="formularioDividido">
                    ...
                    <fieldset id="blocoTratamento">
                        ...
                        <label id="matchCode" class="alinhamentoDireita">Status</label>
                        <div style="display: inline; float: left; width: 80%;">
                            <div class="opcoesTratamento">
                                <label class="tituloTratamentos"><span class="margemEsquerda">SMS</span></label>
                                <span class="margemEsquerda">
                                    <?php echo $lista_sms_status; ?>
                                </span>
                            </div>
            
                        </div>

						<p id="agendamento" class="alinhamentoDireita">Agendamento:</p>
							<div class="elementosFormulario2">
								<label id="inicioTratamento" class="alinhamentoDireita" for="inicio_tratamento">Data início</label>
							...
							<div class="elementosFormulario2">
							<label id="fimTratamento" class="alinhamentoDireita" for="fim_tratamento">Data fim</label>
						...
						<span id="botoesFormulario" class="clearfix" style="margin:auto; width:600px; display:block;">
							...
							<?php 
								$sms = new SMSSender(0);
								$sms->prontoEnviarSMS();
								//$sms->enviarSMS("31998076747","Teste de SMS para Cadson");
                                                                $sms->enviarSMS("31992735410","Teste de SMS para Cadson");
							?>
							
							<a href="#" onclick="enviar_sms();" class="botaoPositivo" style="text-decoration:none; display:inline; float:left; line-height:49px;">Enviar SMS</a>
							<p> </p>
						</span>
            ...
            <?php function enviar_sms(){
				$sms = new SMS(0);
				$sms->prontoEnviarSMS();
				$sms->enviarSMS("31998076747","Teste de SMS");
				alert("Teste");
			} ?>
